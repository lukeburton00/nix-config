{
  ...
  # AeroSpace is macOS-only. The `luke` user is shared between NixOS and
  # darwin hosts, so gate the whole module to keep it inert on Linux.
    lib.mkIf pkgs.stdenv.isDarwin {
      programs.aerospace = {
        enable = true;

        # Manages the launchd agent so AeroSpace starts at login (HM overrides
        # `start-at-login` in the generated config, which requires this).
        launchd.enable = true;
